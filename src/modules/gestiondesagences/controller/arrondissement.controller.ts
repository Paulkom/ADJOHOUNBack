...
export const getAllArrondissement = async (req: Request, res: Response) => {
    ...
    .leftJoinAndSelect('arrondissement.quartiers', 'quartiers')
    ...
    .then(([data, totalElements]) => {
        const message = 'La liste des arrondissements a bien été récupéré.';
        const totalPages = Math.ceil(totalElements / limit);
        return success(res,200,{data, totalPages, totalElements, limit}, message);
    }).catch(error => {
        const message = `La liste des arrondissements n'a pas pu être récupéré. Réessayez dans quelques instants.`
        return generateServerErrorCode(res,500,error,message)
    })
};

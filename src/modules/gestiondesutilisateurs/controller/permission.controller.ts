...
export const getPermissionNotIn = async (req: Request, res: Response) => {
    ...
    var lesPermi = myDataSource.getRepository(Permission)
        .createQueryBuilder("per");
        if(sesPermissions.length > 0){
            lesPermi.where(`per.id NOT IN (:...ids)`, { ids: sesPermissions.map(permission => permission.id) })

        }
    await lesPermi.orderBy("per.nom","ASC").getMany()
    .then(permission => {
        if(permission === null) {
          const message = `La permission demandé n'existe pas. Réessayez avec un autre identifiant.`
          return generateServerErrorCode(res,400,"L'id n'existe pas",message)
